...
_END;
                ...
                $query="SELECT * FROM reservations where p_id='$id' and date >= '$currentDate' ";
                $result=  mysqli_query($con, $query);
                $r= mysqli_num_rows($result);
                if($r==0){
                     echo"<br><h1>NO RESULT!</h1><br><h1>YOU DO NOT Have any reservations</h1>";
                }else{
                    echo<<<_END
               <br></br><table >
                <tr><th>Id of the reservation</th><th>Driver Id</th><th>Driver Name</th><th>Location</th><th>destination</th><th>date</th><th>start Time</th><th>Return Time</th><th>Whatsapp</th><th>Chat</th><th>Rating</th><th>Delete</th></tr>
...
_END;
                ...
                $s=$row['d_id'];
                $query2="SELECT * FROM passenger where id='$s'";
                $result2=  mysqli_query($con, $query2);
                $r2= mysqli_num_rows($result2);
                $row2=mysqli_fetch_assoc($result2);
                echo"<tr><td>".$row['id']."</td><td>".$row['d_id']."</td><td>".$row2['FullName']."</td><td>".$row['start']."</td><td>".$row['end']."</td><td>".$row['date']."</td><td>".$row['time']."</td><td>".$row['timereturn']."</td>";
                     echo "<td><a href ='https://wa.me/".$row2['phoneNumber']."'><img src='whatsapp.png' hieght='50px' width='30px'></a></td><td><a href ='chat.php?d_id=".$row['d_id']."'><img src='email.png' hieght='40px' width='40px'></a></td><td><a href ='rate.php?d_id=".$row['d_id']."'><img src='star1.png' hieght='40px' width='60px'></a></td><td><a href ='deleteres.php?id=".$row['id']."&& d_id=".$row['d_id']."&& dt=".$row['date']."&& time=".$row['time']."'><img src='delete.png' hieght='20px' width='20px'></a></td>"
                        . "</tr>";
...
<body>
    <form method="POST" action="rides.php" enctype='multipart/form-data'>
