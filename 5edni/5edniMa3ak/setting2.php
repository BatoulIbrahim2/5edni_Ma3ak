...
require_once 'connection.php';
 ...
 $id = $_SESSION['id'];
 if (isset($_POST['phone']) && isset($_POST['nphone']) )
{
  $phone=$_POST['phone'];
  $nphone=$_POST['nphone'];
 ...
 $item_query = "select * from passenger where id = '$id'";
 
 $result=  mysqli_query($con, $item_query);
 $row = mysqli_fetch_assoc($result);
 
 $tphone = $row['phoneNumber'];
 if($tphone==$phone){
   
 if($phone!=$nphone){
   
    
            $sql = "update passenger set phoneNumber = '$nphone' where id = '$id'";
...
<body>
   ...
   <div class="topnav" id="t">
    ...
    <a href="logout.php">LOGOUT</a>
      ...
      <br><br>
      <div class="location"><form method="POST" action="setting2.php" >
        <label for="phone number">   Your Current Phone number:   </label>
        <input type="tel" id="phone" name="phone">
     
        
        <br>
        <label for="phone">Your new phone number:</label>
        <input type="tel" id="nphone" name="nphone">
        <br>
