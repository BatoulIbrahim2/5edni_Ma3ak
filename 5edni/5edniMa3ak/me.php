...
$query = "SELECT id,driver_id,start_location, end_location, departure_time,time,timereturn,available_seat,smoking,food,drink,music,ac,gender  FROM ride where driver_id='$id' and departure_time >= '$currentDate' ";
$result = mysqli_query($con, $query);
$r = mysqli_num_rows($result);
if($r==0){
    echo"<br><h1>NO RESULT!</h1><br><h1>YOU SHOULD CREATE A RIDE FIRST</h1>";
}else{
    echo<<<_END
      
     </div><br><br>
<br></br><table>
<tr><th>id</th><th>id</th><th>Location</th><th>Destination</th><th>Date</th><th>Start Time</th><th>Return Time</th><th>Available Seat</th><th>additional-details:</th><th>delete</th></tr>
...
for ($i = 0; $i < $r; $i++) {
    ...
    echo"<tr><td>" .$row['id']."</td><td>". $row['driver_id'] . "</td><td>" . $row['start_location'] . "</td><td>" . $row['end_location'] . "</td><td>" . $row['departure_time'] .
    "</td><td>" . $row['time'] . "</td><td>" . $row['timereturn'] . "</td><td>" . $row['available_seat'] . "</td><td>" . $row['smoking'] . "<br>" . $row['food'] . "<br>" . $row['drink'] . "<br>" . $row['music'] . "<br>" . $row['ac'] . "<br>" . $row['gender'] . "<td><a href ='delete.php?id=" . $row['id'] . "&& d_id=".$row['driver_id']." && dt=".$row['departure_time']."&& time=". $row['time'] . "'><img src='delete.png' "
...
<body>
    <form method="POST" action="passenger.php" enctype='multipart/form-data'>
